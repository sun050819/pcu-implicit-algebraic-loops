# -*- coding: utf-8 -*-
"""run_49_gate1_modrastrigin.py
Gate 1 second positive family: EA + PCU on modulated Rastrigin.

This addresses the "only f5" criticism by testing the same EA+PCU
initialization protocol on a second periodic function family.

Output: results/gate1_modrastrigin.json
"""
...
# ---- Modulated Rastrigin function ----
def f_mod_rastrigin(X):
...
def main():
    print("=" * 60)
    print("GATE 1 SECOND FAMILY: Modulated Rastrigin EA + PCU")
    print("=" * 60)
    print(f"Problem: Modulated Rastrigin, {D}D")
    print(f"Runs per condition: {N_RUNS}")
    print(f"Budget: {BUDGET} FE per run")
    print()

    results = {
        'problem': 'Modulated Rastrigin 30D',
        'budget_fe': BUDGET,
        'n_runs': N_RUNS,
        'seeds': SEEDS,
    ...
    out_path = os.path.join(RESULTS, 'gate1_modrastrigin.json')
    ...
    for idx, sd in enumerate(SEEDS):
        ...
        # 1. CMA-ES pure
        cma_f, cma_hit, cma_fe = cmaes_from(f_mod_rastrigin, x0, BUDGET, sd)
        ...
        t_pcu = time.time()
        x_cand, status = estimate_and_pcu(f_mod_rastrigin, grad_mod_rastrigin, hess_mod_rastrigin, x0, tol=1e-4)
        pcu_triggered = status is not None
        print(f"  PCU: {'triggered' if pcu_triggered else 'no trigger'} ({time.time()-t_pcu:.1f}s)")

        if pcu_triggered:
            # 3. CMA-ES + PCU init
            cma_pcu_f, cma_pcu_hit, cma_pcu_fe = cmaes_from(f_mod_rastrigin, x_cand, BUDGET, sd)
            ...
            # 4. SHADE + PCU init
            sh_pcu_f, sh_pcu_hit, sh_pcu_fe = shade_optimize(f_mod_rastrigin, x_cand, BUDGET, sd)
        ...
        # 5. SHADE pure
        sh_pure_f, sh_pure_hit, sh_pure_fe = shade_optimize(f_mod_rastrigin, x0, BUDGET, sd)
    ...
    # Summary
    n_trig = sum(1 for r in results['cmaes_pcu_init'] if r.get('pcu_triggered'))
    print(f"\n=== Summary ===")
